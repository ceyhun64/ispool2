...
export default function HomePageManagement() {
  ...
  // Showcase (Özel Üretim Örnek Çalışmalar) states
  const [showcaseForm, setShowcaseForm] = useState({
    title: "",
    order: 0,
  });
  const [showcaseImage, setShowcaseImage] = useState<File | null>(null);
  const [showcaseImagePreview, setShowcaseImagePreview] = useState<string>("");
  ...
  const fetchShowcaseImages = async () => {
    try {
      ...
      const data = await res.json();
      if (res.ok) setShowcaseImages(data.images);
    } catch (err) {
      toast.error("Örnek çalışma verileri alınamadı.");
    }
  };
  ...
  const handleShowcaseImageChange = (
    ...
    if (file) {
      ...
      const reader = new FileReader();
      reader.onloadend = () => {
        setShowcaseImagePreview(reader.result as string);
      };
      reader.readAsDataURL(file);
    }
  };
  ...
  // Showcase image ekleme
  const handleAddShowcaseImage = async () => {
    if (!showcaseImage) {
      return toast.error("Resim yüklemek zorunludur.");
    }
    ...
    try {
      ...
      if (showcaseForm.title.trim())
        formData.append("title", showcaseForm.title.trim());
      formData.append("order", showcaseForm.order.toString());
      formData.append("image", showcaseImage);
      ...
      const data = await res.json();
      if (res.ok) {
        setShowcaseImages((prev) => [...prev, data.image]);
        setShowcaseForm({ title: "", order: 0 });
        setShowcaseImage(null);
        setShowcaseImagePreview("");
        toast.success("Örnek çalışma eklendi.");
      } else {
        toast.error(data.message || "Ekleme başarısız.");
      }
    } catch (err) {
  ...
  return (
    ...
    [&::-webkit-inner-spin-button]:cursor-pointer"
        ...
        {/* SHOWCASE TAB - Özel Üretim Örnek Çalışmalar */}
              ...
              <Card className="bg-white border-slate-200 sticky top-6 rounded-sm">
                <CardContent className="p-6 space-y-4">
                  ...
                  <div className="space-y-4">
                    ...
                    <div>
                      <label className="text-xs font-semibold text-slate-700 mb-2 block">
                        Çalışma Resmi <span className="text-red-500">*</span>
                      </label>
                    ...
                    <div>
                      ...
                      <Input
                        placeholder="Örn: Kurumsal İş Elbisesi"
    ...
    [&::-webkit-inner-spin-button]:cursor-pointer"
                    ...
                    <Button
                      ...
                      disabled={isAdding || !showcaseImage}
                      className="w-full bg-slate-900 hover:bg-slate-800 text-white rounded-full"
                    >
                      {isAdding ? (
                        <Loader2 className="w-4 h-4 animate-spin" />
            ...
            <div className="lg:col-span-2 space-y-4">
              <div className="flex items-center justify-between mb-4">
                <h3 className="text-sm font-semibold text-slate-700 flex items-center gap-2">
                  <Eye className="w-4 h-4" /> Aktif Örnek Çalışmalar
                </h3>
              </div>
              ...
              {isLoading ? (
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                    ...
                    >
                      ...
                      <div className="absolute top-2 right-2 opacity-0 group-hover:opacity-100 transition-opacity">
                        <Button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            setItemToDelete({ type: "showcase", id: item.id });
                            setDeleteDialogOpen(true);
                          }}
                        >
                          <Trash2 className="w-4 h-4" />
                  ...
                  {showcaseImages.length === 0 && (
                    <div className="col-span-full h-40 flex items-center justify-center text-sm text-slate-400 bg-white rounded border">
