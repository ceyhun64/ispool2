  ...
  return (
    <div className="group flex flex-row w-full gap-3 md:gap-5 py-4 md:py-6 px-2 md:px-3 bg-white border-b border-slate-100 last:border-0 transition-all">
      ...
      <div className="flex-1 flex flex-col justify-between min-w-0">
        {" "}
        {/* min-w-0 text-truncate için önemli */}
        ...
        {/* Adet ve Fiyat Bölümü - Mobilde dikey, tablette yatay olabilir */}
        <div className="flex flex-row justify-between items-end mt-2 md:mt-4">
          ...
          <div className="flex items-center bg-slate-50 border border-slate-200">
            <button
              onClick={onDecrease}
              disabled={quantity <= 1}
              className="px-1.5 md:px-2 py-1 text-slate-500 hover:bg-slate-100 disabled:opacity-30 transition-all"
            >
              <Minus size={12} strokeWidth={2.5} />
            </button>
            <span className="w-8 md:w-10 text-center text-[11px] font-bold text-slate-900 font-mono">
              {quantity}
            </span>
            <button
              onClick={onIncrease}
              className="px-1.5 md:px-2 py-1 text-slate-500 hover:bg-slate-100 transition-all"
            >
          ...
          <div className="text-right ml-2">
            ...
            <span className="text-[13px] sm:text-sm md:text-base font-black tracking-tighter text-slate-900 font-mono">
              ₺
