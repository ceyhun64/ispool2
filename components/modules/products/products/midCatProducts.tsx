// /components/modules/products/products/midCatProducts.tsx
"use client";
...
interface MidCatProductsProps {
  id: number;
  midId: number;
  showDiscountOnly?: boolean;
}

export default function MidCatProducts({
  id,
  midId,
  ...
  useEffect(() => {
    async function fetchCategory() {
      try {
        const res = await fetch(`/api/category/${id}/${midId}`);
  ...
  useEffect(() => {
    async function fetchProducts() {
      try {
        setLoading(true);
        const url = `/api/products/category/${id}/${midId}`;
