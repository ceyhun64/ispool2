...
import { ArrowRight, Drill, Cpu, ScanEye, Layers } from "lucide-react";
  ...
  return (
    <section className="bg-slate-50 py-24 relative overflow-hidden">
      ...
      <div className="max-w-[1600px] mx-auto px-6 md:px-12 relative z-10">
        {/* Başlık Alanı - Endüstriyel Hiyerarşi */}
        <div className="mb-16 flex flex-col lg:flex-row lg:items-end justify-between gap-10">
          <div className="space-y-4">
            <div className="flex items-center gap-3">
              ...
              <span className="text-orange-600 font-black tracking-[0.3em] text-[10px] uppercase">
                İnovasyon Merkezi v2.0
              </span>
            </div>
            <h2 className="text-4xl md:text-3xl font-black tracking-tighter text-slate-900 leading-none uppercase">
          ...
          <div className="max-w-md border-l border-white/10 pl-8 py-2">
            <p className="text-slate-600 text-sm md:text-sm font-medium leading-relaxed">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-10">
          {loading
            ? Array.from({ length: 2 }).map((_, i) => (
                <Skeleton
                  key={i}
                  className="aspect-[16/9] w-full rounded-none bg-white/5"
                />
              ))
...
function ModernCategoryCard({ category }: { category: CategoryData }) {
  return (
    <Link
      href={category.href}
      className="group relative block overflow-hidden bg-black aspect-[16/9] border border-white/5 shadow-2xl"
    >
      {/* Görsel Katmanı - Daha Karanlık ve Teknik */}
      <div className="relative w-full h-full">
        ...
        />
        {/* Overlay - Cyberpunk/Industrial Gradient */}
        <div className="absolute inset-0 bg-gradient-to-br from-slate-950/80 via-transparent to-orange-950/20" />
      </div>

      {/* İçerik Katmanı - Operasyon Paneli Havası */}
      <div className="absolute inset-0 p-10 md:p-14 flex flex-col justify-between z-20">
        <div className="flex justify-between items-start">
          <div className="bg-orange-600 p-4 shadow-xl shadow-orange-900/20 -translate-y-2 group-hover:translate-y-0 transition-transform duration-500">
            {category.id === 1 ? (
          ...
          <div className="flex flex-col items-end opacity-40">
            ...
            <Layers size={14} className="text-white mt-1" />
          </div>
        </div>
        ...
        <div className="space-y-6">
          <div className="relative">
            ...
            <div
              className={`absolute -left-14 top-1/2 w-10 h-[2px] bg-orange-600 opacity-0 group-hover:opacity-100 transition-all duration-500`}
            />
          </div>

          <p className="text-slate-300 text-sm md:text-base max-w-sm font-semibold leading-snug tracking-tight">
            {category.subTitle}
          </p>

          <div className="flex items-center gap-6 pt-6">
            <div className="flex items-center gap-3 text-[10px] font-black text-white uppercase tracking-[0.3em] group-hover:text-orange-500 transition-colors duration-300">
              HEMEN KEŞFET
      ...
      {/* Tarayıcı Çizgisi Animasyonu (Scan Line) */}
